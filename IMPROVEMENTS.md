...
**Γνωστό υπολειπόμενο κενό (κλείστηκε την επόμενη μέρα — δες Β12):** ένα
*retained* MQTT μήνυμα (π.χ. σε reconnect της εφαρμογής) ξαναπροχωρούσε το
lastSeen σε "τώρα" ακόμα κι αν ήταν ώρες παλιό — το πρωτόκολλο δεν
κουβαλούσε καθόλου πραγματικό timestamp ώστε να ξεχωρίσει live delivery από
retained replay.
...
### Β11. Το `pi/tools/show_qr.py` παρήγαγε QR χωρίς `api_token`/σωστό fingerprint **[εύκολο]** — ✅ έγινε (2026-08-11)
Το ίδιο πρόβλημα με το Β10, από άλλη πόρτα. Το script έπαιρνε
`--pass`/`--user`/`--port` ως χειροκίνητα ορίσματα και υπολόγιζε το δικό του
TLS fingerprint με `openssl x509` πάνω σε ένα cert αρχείο — δεν ενημερώθηκε
ποτέ μετά το πέρασμα 2026-07-28 που πρόσθεσε `api_token`/`cam_token` στο
πραγματικό `_pairing_payload()` του portal, ούτε μετά το `79e37ff` που έκανε
το `tls_fingerprint` comma-separated λίστα (CA+leaf, όχι ένα). Ένα QR από
αυτό το script ζευγάρωνε "επιτυχώς" αλλά με άδειο `apiToken` (ίδιο ψευδές
security alert με το Β10) **και** με fingerprint που το certificate pinning
της εφαρμογής δεν θα ταίριαζε ποτέ.

Ξαναγράφτηκε να διαβάζει ακριβώς τα ίδια αρχεία με το `_pairing_payload()`
(`device.json`, `hivemq.json`, `cam_token.txt`) — δεν μπορεί πια να
ξεσυγχρονιστεί σιωπηλά. Προστέθηκε και auto-detection της τρέχουσας LAN IP
(UDP socket routing trick, δεν στέλνει πραγματικό πακέτο) αντί για χειροκίνητο
`--lan` — στην τοπολογία "Pi πάνω στο hotspot του κινητού" η IP του Pi
αλλάζει ανά DHCP lease, οπότε ένα παλιό/τυπωμένο QR είχε καλές πιθανότητες
να κουβαλάει ήδη λάθος IP. `pi/tests/test_show_qr.py` (5 tests) επαληθεύει
ότι το payload ταιριάζει ακριβώς με τα πεδία που περιμένει
`ConnectionConfig.fromJson` στην εφαρμογή.

### Β12. "Last seen" έδειχνε την ώρα ανοίγματος της εφαρμογής σε **όλους** τους κόμβους **[μέτριο]** — ✅ έγινε (2026-08-12)
Το υπολειπόμενο κενό του Β8, κλεισμένο στη ρίζα του. Το MQTT ξαναστέλνει
**κάθε** retained topic ολόκληρο σε κάθε (re)connect, και το πρωτόκολλο δεν
λέει πουθενά πόσο παλιό είναι ένα μήνυμα. Άρα η εφαρμογή χρονολογούσε κάθε
κόμβο από τη στιγμή που έφτασε το replay — δηλαδή κάθε κόμβος έδειχνε «είδα
τον μόλις τώρα» κάθε φορά που άνοιγες την εφαρμογή, ό,τι κι αν είχε συμβεί
στην πραγματικότητα.

Η λύση δεν ήταν στο UI αλλά στην πηγή: το **`serial_bridge.py` σφραγίζει
πλέον `ts`** (epoch seconds) μέσα σε κάθε `/mesh` payload. Επειδή το topic
είναι retained, **η σφραγίδα της τελευταίας πραγματικής παρατήρησης είναι
αυτή που επιβιώνει στον broker** — ακριβώς η τιμή που θέλει το "last seen",
και παραμένει αληθής μέσα από ένα replay. Συμπληρωματικά στην εφαρμογή:

- Το MQTT `retain` flag περνάει τώρα από το `mqtt_connection.dart` μέσω του
  `NodeStatus` ως το merge του repository, ώστε ένα σκέτο replay να μην
  μπορεί να περάσει για απόδειξη ζωής.
- Το `NodeStatus.lastSeen` έγινε nullable: το «ποτέ δεν επιβεβαιώθηκε
  ζωντανός» είναι πραγματική κατάσταση, διαφορετική από μια ώρα, και
  εμφανίζεται ως `Unknown` αντί για επινοημένο timestamp.
- Το merge κρατά το **μεταγενέστερο** από τα δύο γνωστά timestamps: τα
  retained topics ξαναπαίζουν με αυθαίρετη σειρά, και το last-seen οφείλει να
  είναι μονότονο (ποτέ προς τα πίσω).

**Κενό που βρέθηκε δοκιμάζοντας το ίδιο το fix:** το firmware στέλνει το root
record της γέφυρας μόνο από το `setup()`, οπότε ένα restart του
`serial_bridge` με το ESP32 ήδη σε λειτουργία δεν θα έβλεπε ποτέ mesh γραμμή
και δεν θα ανανέωνε ποτέ — δηλαδή η συνήθης περίπτωση κάθε redeploy. Τώρα
συνθέτει το canonical record (`parent: null, rank: 0` — ο ίδιος ο ορισμός του
firmware για τον εαυτό του) ως fallback, με throttle 60s.

Επαληθεύτηκε στο bench Pi. Οι δύο edge nodes εξακολουθούν να δείχνουν
`Unknown` και **αυτό είναι σωστό**: δεν έχουν αναφέρει από την αναβάθμιση,
άρα το Pi ποτέ δεν κατέγραψε πότε τους είδε τελευταία. Το να επινοηθεί ώρα
εκεί θα ήταν το bug, όχι η διόρθωση.

### Β13. Το drag-to-pin στο mesh map δεν έκανε τίποτα, και το "Unpin" φαινόταν χαλασμένο **[μέτριο]** — ✅ έγινε (2026-08-12)
Το `_buildCard` εναλλασσόταν μεταξύ `Positioned` και `AnimatedPositioned` —
**διαφορετικοί τύποι widget** — τη στιγμή ακριβώς που ξεκινούσε το drag, χωρίς
κανένα `Key`. Το `Element.canUpdate` επιστρέφει false σε αλλαγή τύπου, οπότε
το Flutter κατέστρεφε και ξανάφτιαχνε το subtree, σκοτώνοντας τον ζωντανό
`GestureRecognizer` **στη μέση της χειρονομίας**. Το `onPanStart` προλάβαινε
να γράψει την **προ-drag** θέση, αλλά τα `onPanUpdate`/`onPanEnd` της ίδιας
αφής δεν έφταναν ποτέ — άρα το "pin" κατέγραφε ακριβώς εκεί που ήταν ήδη η
κάρτα, οπτικά αδιάκριτο από το auto-layout, και το "Unpin" δεν είχε τίποτα
ορατό να αναιρέσει.

Διορθώθηκε με σταθερό `ValueKey(nodeId)` και **έναν** αμετάβλητο τύπο widget
(animation μέσω `duration: Duration.zero` αντί για εναλλαγή κλάσης). Το
regression test οδηγεί πραγματική πολυβηματική χειρονομία και επαληθεύτηκε ότι
**αποτυγχάνει** στον παλιό κώδικα — ένα single-jump `tester.drag()` δεν το
έπιανε, γιατί μόνο μια χειρονομία με πολλά βήματα εκθέτει τον νεκρό
recognizer.

### Β14. Η εφαρμογή μιλούσε στον χρήστη σαν debugger **[μέτριο]** — ✅ έγινε (2026-08-12)
Πλήρες πέρασμα UX για μη-τεχνικό χρήστη (καλλιεργητή). Μόνο app — κανένα
firmware, καμία αλλαγή πρωτοκόλλου. Τα χειρότερα ευρήματα, όλα επαληθευμένα
στον κώδικα πριν διορθωθούν:

- **`settings_screen.dart:41` τύπωνε shell command** στον χρήστη:
  `'$code\nMust match the Pi: sudo bash scripts/selftest.sh'`. Είναι οδηγία
  διαχειριστή· μετακινήθηκε στο `SECURITY.md`.
- **`Text('Error: $e')`** ως ολόκληρο το UI σφάλματος σε 4 οθόνες — δηλαδή
  `SocketException: OS Error: ...` στο πρόσωπο του χρήστη.
- **Το "Disconnect" έσβηνε το pairing με ένα tap**, μια γραμμή κάτω από
  navigation item, χωρίς undo (η μόνη επιστροφή είναι το QR).
- **MAC ως τίτλος συσκευής**, `pump1` ως όνομα διακόπτη, `s.name` (ωμό enum)
  ως κατάσταση σύνδεσης.

Νέα αρχεία: `services/device_names_store.dart` (μετονομασία συσκευών, ίδιο
pattern με `NodePositionsStore`), `utils/display_name.dart`,
`utils/plain_language.dart`, `utils/friendly_error.dart`,
`screens/common/friendly_error_view.dart`, `screens/common/rename_device_dialog.dart`.

**Το σημαντικότερο τεχνικά:** τα κατώφλια σταμάτησαν να αντιγράφονται. Τα
−60/−75 ζουν στο `plain_language.dart` και τα εισάγει ο `linkQualityOf`· τα
80/50/20 ομοίως ο `batteryIconFor`. Δύο tests επιβεβαιώνουν ότι η λέξη και το
χρώμα/εικονίδιο αλλάζουν ζώνη στις **ίδιες ακριβώς** τιμές — αποτυγχάνουν αν
κάποιος αλλάξει νούμερο σε ένα μόνο σημείο, που είναι ακριβώς έτσι που ένα
legend καταλήγει να διαφωνεί με τη γραμμή δίπλα του.

Επίσης: η σειρά των branches στο `describeError` ελέγχεται από test — το
`HandshakeException` **είναι** `IOException`, οπότε χωρίς σωστή σειρά μια
πιθανή πλαστοπροσωπία θα αναφερόταν ως γενική αποτυχία σύνδεσης.

Τα ωμά δεδομένα (dBm, rank, MAC, mV) **δεν αφαιρέθηκαν** — μπήκαν σε collapsed
"Technical details", γιατί είναι ακριβώς αυτά που συγκρίνεις με έξοδο
`mosquitto_sub` όταν κάτι δεν πάει καλά.

**Δύο μηνύματα ήταν ενεργά παραπλανητικά:** το «βεβαιώσου ότι είσαι στο ίδιο
WiFi» (στην τοπολογία hotspot **είσαι**, και το mDNS απλά δεν μπορεί να
δουλέψει — ο `MulticastLock` του Android αφορά το client radio, όχι το SoftAP)
και το «κάποιος μπορεί να μιμείται το θερμοκήπιό σου» ως πρώτη εξήγηση, ενώ η
συντριπτικά πιθανότερη αιτία είναι ότι ο ίδιος ο ιδιοκτήτης επανεγκατέστησε το
hub. Και τα δύο ξαναγράφτηκαν· η προειδοποίηση ασφαλείας παραμένει, απλώς
δεύτερη.

`flutter test`: 279 passed (από 213). `flutter analyze` καθαρό.

### Β15. Τα ghost retained topics «επέστρεφαν» μετά από κάθε καθάρισμα **[μέτριο]** — ✅ έγινε (2026-08-12)
Ένας παροπλισμένος κόμβος (`zone1_test`, MAC `75EC`) εμφανιζόταν επίμονα ως
φάντασμα συσκευή **και** ως φάντασμα ζώνη στο dashboard, παρότι καθαρίστηκε
τρεις φορές. Η αρχική διάγνωση («το mosquitto persistence δεν πρόλαβε να
γράψει πριν το reboot») ήταν **λάθος**. Ο πραγματικός μηχανισμός,
επαληθευμένος με ελεγχόμενο πείραμα:

Το MQTT παραδίδει ένα *live* μήνυμα σε υπάρχοντα subscriber με το **retain
flag καθαρισμένο** — τίθεται μόνο όταν ένα μήνυμα αναπαράγεται σε *νέα*
συνδρομή. Ο `hivemq_bridge.py` προωθεί `retain=msg.retain`, οπότε ένα retained
publish — **συμπεριλαμβανομένου του empty-payload publish που σημαίνει
«διαγραφή»** — περνά τη γέφυρα ως συνηθισμένο, μη-retained μήνυμα. Το
αποθηκευμένο αντίγραφο του άλλου broker μένει **ανέγγιχτο**.

Άρα η retained *κατάσταση* συγχρονίζεται **μόνο σε reconnect** της γέφυρας,
όταν παραλαμβάνει το retained σύνολο της άλλης πλευράς με `retain=1`. Αυτό
ανάσταινε τα ghosts: καθάριζα τοπικά, η γέφυρα αργότερα επανασυνδεόταν,
κατέβαζε το ακόμα βρώμικο σύνολο του cloud, και το ξαναδημοσίευε τοπικά
**με** retain.

**Πώς επαληθεύτηκε:** φύτεψα topic μόνο στο cloud· δεν εμφανίστηκε τοπικά για
λεπτά, και εμφανίστηκε **τη στιγμή** που έγινε restart η γέφυρα. Πρόβλεψη που
διατυπώθηκε πριν το πείραμα και επιβεβαιώθηκε.

Νέο `pi/scripts/clear_retained.sh`: ανακαλύπτει τα συγκεκριμένα topics πίσω
από ένα wildcard (δεν μπορείς να διαγράψεις με wildcard), καθαρίζει **cloud
πρώτα και μετά τοπικά** (ώστε ένα reconnect στη χειρότερη στιγμή να μπορεί
μόνο να κατεβάσει ήδη καθαρό σύνολο), στέλνει `SIGUSR1` στο mosquitto για
άμεση εγγραφή του persistence (default autosave: 30 λεπτά — power cut μέσα σε
αυτό το παράθυρο ανέτρεπε τη δουλειά), και **επαληθεύει και τους δύο brokers**
στο τέλος αντί να υποθέτει επιτυχία. `DRY_RUN=1` για προεπισκόπηση.

Δοκιμάστηκε end-to-end στο bench Pi, συμπεριλαμβανομένου restart της γέφυρας
μετά τον καθαρισμό — ακριβώς η ενέργεια που πριν ανάσταινε τα ghosts.
