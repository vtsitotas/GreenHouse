...
3. **TLS στη ζεύξη της κάμερας** (`:8090` και το ESP32 HTTP API) — η μόνη
   plaintext ζεύξη που απομένει. Ο ESP32 δεν έχει πρακτικό τρόπο να
   επικυρώσει per-unit self-signed cert χωρίς provisioning στο firmware·
   χρειάζεται σχεδιασμό, όχι απλή διόρθωση. Βλ. `10-security.md §8.2`.
4. **Β3 (LAN streaming μπλοκάρει motion detection)** — άμεσα σχετικό με το
   τρέχον camera bench-testing (βλ. `TODO.md` §3), αλλά χρειάζεται πραγματικό
   hardware για να επαληθευτεί οποιαδήποτε διόρθωση.
5. **Δ2 (log helper)** — καθαρά καλαισθητικό, χαμηλή προτεραιότητα.
