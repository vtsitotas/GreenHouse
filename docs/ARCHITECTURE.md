...
- Τα γραφήματα ιστορικού δουλεύουν και εκτός LAN: η εφαρμογή στέλνει το ερώτημα
  μέσω MQTT (`greenhouse/history/request` → `greenhouse/history/response/<id>`,
  απαντά το `greenhouse-recorder`) αντί για το HTTP :80, το οποίο δεν
  αναμεταδίδεται μέσω HiveMQ. Η εφαρμογή διαλέγει HTTP όταν είναι στο LAN,
  MQTT όταν είναι εκτός.
