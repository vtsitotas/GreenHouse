...
- **Κανάλι — σταθερό, όχι πια σάρωση router:** το ESP-NOW δεν έχει δικό του
  κανάλι· χρησιμοποιεί όποιο κανάλι είναι ήδη συντονισμένο το radio του
  chip. Μέχρι την υλοποίηση του UART-wired bridge (§δες `04-bridge-gateway.md`),
  κάθε κόμβος σάρωνε (`WiFi.scanNetworks()`) το SSID του σπιτικού router
  μόνο για να συμφωνήσει σε κανάλι, χωρίς ποτέ association. Τώρα που η
  γέφυρα δεν συνδέεται καθόλου σε WiFi (UART προς το Pi αντί για MQTT),
  **δεν υπάρχει πια router να σαρωθεί** — κάθε κόμβος (γέφυρα + edge nodes)
  κλειδώνει σε ένα σταθερό κανάλι, `MESH_FIXED_CHANNEL`
  (`firmware/libraries/GreenhouseMesh/mesh_config.h`), μέσω
  `esp_wifi_set_channel()` χωρίς ποτέ σάρωση. Ο κάθε edge node **δεν
  συνδέεται ποτέ** στο WiFi (`WiFi.disconnect()` αμέσως μετά το
  `WiFi.mode(WIFI_STA)`) — μπαίνει σε promiscuous mode μόνο για να θέσει το
  κανάλι, ίδιο μοτίβο με πριν, απλά χωρίς το ενδιάμεσο σάρωσης βήμα. Αφού
  δεν υπάρχει πια router να αλλάξει κανάλι, το παλιό `MESH_RESCAN_AFTER_MS`
  re-scan-on-timeout μηχανισμό παραμένει στον κώδικα ως αβλαβές no-op
  (ξαναθέτει το ίδιο σταθερό κανάλι) αντί να αφαιρεθεί εντελώς — μηδενικό
  ρίσκο, απλά πια χωρίς πρακτικό σκοπό.

## 3. ADC — Ανάγνωση υγρασίας εδάφους

- Pin: `SOIL_DATA_PIN = 1` → αντιστοιχεί σε `ADC1_CH1` στο ESP32-C3. (Όχι
  GPIO2: strapping pin του C3, αρκετές πλακέτες έχουν pull-up εκεί που
  "κολλάει" το ADC κοντά στο VCC ανεξάρτητα από τον αισθητήρα — βρέθηκε με
  `firmware/sensor_pin_test/sensor_pin_test.ino`.)
...
> ⚠️ **Αλλαγή pin (2026-08-10): GPIO2 → GPIO1.** Κόμβοι που καλωδιώθηκαν με
> την παλιά τιμή χρειάζονται **και** μετακίνηση του καλωδίου AOUT στο GPIO1
> **και** επαναπρογραμματισμό. Σύμπτωμα της παλιάς καλωδίωσης: `analogRead()`
> κολλημένο στο 4095 μόνιμα, χωρίς καμία αντίδραση όταν βρέχεται ο αισθητήρας.

### 3.1 Bench test καλωδίωσης — `firmware/sensor_pin_test/`

Standalone sketch (χωρίς WiFi/ESP-NOW/mesh) που τροφοδοτεί τους δύο αισθητήρες
και τυπώνει raw + μετατρεμμένες τιμές στο Serial (115200) κάθε 0.5 s (ADC) και
2.5 s (DHT22). Χρησιμεύει για έλεγχο κολλήσεων: κουνώντας κάθε επαφή, μια
διακοπή ή "παγωμένη" τιμή δείχνει ποια κόλληση φταίει. Επισημαίνει αυτόματα
ADC τιμές κολλημένες στα άκρα (`<= 5` ή `>= 4090`) και μετράει σερί αποτυχιών
του DHT22. Τα PWR pins μένουν μόνιμα HIGH (σε αντίθεση με το production
firmware που τα κόβει μεταξύ μετρήσεων) ώστε να μετράς με πολύμετρο ζωντανά.
