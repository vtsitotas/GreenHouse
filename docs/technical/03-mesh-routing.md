...
| `meshDedup[]` | δαχτυλίδι 32 εγγραφών `(mac, seq, ms)` για de-dup — κάθε εγγραφή λήγει μετά από `MESH_DEDUP_WINDOW_MS` (βλ. §7) |
...
`meshDedupSeen()` (`mesh_node.h`): δαχτυλίδι 32 εγγραφών
`(origin_mac, seq, ms)`. Κάθε φορά που ένα πακέτο περνά (είτε ως relay είτε
στη γέφυρα), ελέγχεται αν το ζευγάρι έχει ξαναφανεί **μέσα στα τελευταία
`MESH_DEDUP_WINDOW_MS` (30 s)** — αν ναι, πετιέται και **καταγράφεται** στο
serial log. Αυτό προστατεύει από αντίγραφα λόγω route-flap (π.χ. ο κόμβος
Β άλλαξε γονέα ανάμεσα στην αποστολή ενός πακέτου και ενός retry).

**Γιατί λήγουν οι εγγραφές:** το `seq` ταυτοποιεί μια μέτρηση μόνο όσο ο
κόμβος-πηγή διατηρεί τη RTC μνήμη του. Οποιαδήποτε απώλεια ρεύματος
(powerbank που κόβει στο deep sleep, brownout, αλλαγή μπαταρίας) το
ξαναρχίζει από το 0, οπότε μια cache χωρίς λήξη θα θεωρούσε **κάθε** επόμενη
μέτρηση εκείνου του κόμβου διπλότυπη και θα την πετούσε για πάντα — με μόνη
ανάκαμψη reboot της γέφυρας. Το παράθυρο πρέπει να μένει πάνω από
`MESH_WAKE_MAX_AWAKE_MS` (ένας κύκλος αφύπνισης ξαναστέλνει σκόπιμα το ΙΔΙΟ
`seq` μετά από ανεπιβεβαίωτο tx, και αυτό το retry πρέπει να συνεχίσει να
γίνεται de-dup) και κάτω από `MESH_SLEEP_INTERVAL_MS` (ώστε το ανακυκλωμένο
`seq` ενός κόμβου που έκανε restart να έχει λήξει μέχρι την επόμενη αφύπνισή
του).

Γνωστό tradeoff: μια μέτρηση που έμεινε στο `meshBuf[]` και ξαναστέλνεται
κύκλους αργότερα κρατά το αρχικό της `seq`, οπότε αν η πρώτη απόπειρα είχε
όντως φτάσει, μπορεί να καταγραφεί δύο φορές. Μία διπλή μέτρηση είναι
προτιμότερη από μόνιμη απώλεια όλων των μετρήσεων ενός κόμβου.
