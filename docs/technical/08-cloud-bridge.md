...
### 4α. Η retained κατάσταση **δεν** γεφυρώνεται συνεχώς — μόνο σε reconnect

Μη προφανής και μας κόστισε μια ολόκληρη συνεδρία κυνηγώντας ένα ghost
`zone1_test` (2026-08-12, δες `IMPROVEMENTS.md` Β15):
...
συνδρομή. Επειδή η γραμμή παραπάνω προωθεί `retain=msg.retain`, ένα retained
publish περνά τη γέφυρα ως **μη-retained** μήνυμα. Ο άλλος broker το
παραδίδει στους subscribers του κανονικά, αλλά **δεν το αποθηκεύει**.

Πρακτικές συνέπειες:

- Η retained κατάσταση των δύο brokers συγχρονίζεται **μόνο** όταν η γέφυρα
  (επαν)συνδέεται και παραλαμβάνει το retained σύνολο της άλλης πλευράς με
  `retain=1` — τότε το προωθεί *με* retain.
- Ένα empty-payload publish (= «διαγραφή retained») **δεν διαγράφει** το
  αντίγραφο του άλλου broker. Πρέπει να διαγραφεί ρητά και στις δύο πλευρές,
  αλλιώς το επόμενο reconnect της γέφυρας το επαναφέρει.

Γι' αυτό υπάρχει το `pi/scripts/clear_retained.sh`: καθαρίζει cloud→local και
επαληθεύει και τα δύο.
