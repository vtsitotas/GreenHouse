...
- `pi/scripts/firewall.sh` — TCP port `8090` (cam-bridge intake) was removed
  from `TCP_PORTS`; an open port with nothing behind it is the exact case that
  file's default-deny exists to prevent. Add it back when restoring, and swap
  `pi/tests/test_firewall_rules.py`'s
  `test_cam_bridge_port_is_closed_while_the_camera_is_parked` back for the
  `test_cam_bridge_port_is_allowed` check (plus `8090` in its `justified` set).
- `pi/scripts/selftest.sh` — the `/cam/frame` 401 assertion now only runs if
  something is actually listening on 8090, and an absent `cam_token.txt` is a
  pass rather than a failure. Both revert to unconditional on restore.
